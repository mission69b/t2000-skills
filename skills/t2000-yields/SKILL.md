...
description: >-
  ...
  USDC and USDsui on NAVI, liquid staking APY on VOLO, and the user's
  current earning positions. Use when asked "what's the best yield?",
  "where should I park USDC?", "should I save USDC or USDsui?", "what's my
  earning rate?", or "compare yields". Pairs with t2000-save (action) and
  t2000-rebalance (multi-leg planning).
license: MIT
metadata:
  author: t2000
  version: "1.0"
...
Surface every earning opportunity the user has on t2000 — current pool APYs (NAVI USDC / USDsui save + borrow, VOLO SUI staking), the user's open positions, and earnings so far. Helps the user choose where to deploy idle funds without dragging them through five different commands.
...
5. **Match the user's asset.** If they hold USDC, lead with the USDC pool. If they hold SUI and ask about yield, surface both the swap-and-save path (likely better) and the stake path (keeps SUI exposure).

## Commands

```bash
# Cross-protocol, cross-asset APY snapshot
t2000 rates
...
SUI staking   Stake: 3.47% APY                       (VOLO)
```
...
**User says "swap and save my SUI":**
- This is a `t2000-swap` + `t2000-save` flow, not just a rates query. Route them to that bundle.
- BUT first surface the comparison: VOLO stake (~3.5% APY) vs. swap-to-USDC-and-save (~5.1% APY net of swap fee). Many users prefer keeping SUI exposure.
...
- **Don't conflate save and stake.** They are different products with different risk profiles. See the table in `t2000-stake` for the explicit comparison.

## Related skills

- `t2000-save` — execute the save once a decision is made.
- `t2000-stake` — SUI → vSUI yield.
