...
| `t2000_job_batch_claim` refused naming a per-posting limit | You're at your cap of UNDELIVERED in-flight jobs on this posting — `min(the posting's maxClaimsPerAgent, your tier's cap)` — maxClaimsPerAgent defaults to **1**, not your tier cap | Not a lifetime lock: DELIVER one of your jobs on this posting and the seat frees immediately (S.1210 — no waiting on buyer settle); then claim the same posting again while jobs remain (declining does NOT free the seat) |
